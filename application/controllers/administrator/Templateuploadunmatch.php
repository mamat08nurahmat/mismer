...
//update table unmatch
//delete template

	public function bulk_update(){
		$db_unmatch = db_get_all_data('templateuploadunmatch');
		// print_r($db_unmatch[0]->CHANNEL);
		// print_r(count($db_unmatch));
//2865

for ($x = 0; $x <= 100; $x++) {

// print_r($db_unmatch[$x]->CHANNEL);
// print_r('</br>');


$update_bulk = $this->update_mismerdetal($db_unmatch[$x]->MID,$db_unmatch[$x]->CHANNEL);
print_r($update_bulk);
print_r('</br>');
print_r($db_unmatch[$x]->MID);
print_r('</br>');
...
/*
foreach ($db_unmatch as $key => $value){
	
	// print_r($key);
	// print_r('</br>');
	// print_r('KEY : '.$key.'--> Value : '.$value->WILAYAH);
	// print_r('</br>');
...
// $mid='316035807';
// $channel='XXX';
// $update = $this->update_mismerdetal($mid,$channel);

// print_r($update);

	}

	function update_mismerdetal($mid,$channel){
...
$this->db->update('mismerdetail', $data);
...
return $this->db->affected_rows();
	...
	public function delete($id = null)
	{
		...
		if ($remove) {
            set_message($id.'____'.cclang('has_been_deleted', 'templateuploadunmatch'), 'success');
        } else {
